      ...
      <Row utils="g-gs">
        <Col xxl="12">
          <CardBody class="flex-grow-0 py-2">
            <CardTitleGroup>
              <CardTitle tag="div">
                <h3 class="title mb-3">Tenants</h3>
              </CardTitle>
              <CardTools>
              <Button type="button" variant="info" soft class="mx-1" style="float:right;" as="RouterLink" to="/admin/tenant/create">
                <Icon icon="plus"/>  
                Create
              </Button> 
              </CardTools>
                ...
                <DataTable id="datatable-init-2" class="table-border" v-if="users !== null">
                  <TableHead>
                      <tr>
                          ...
                          <th><OverlineTitle tag="span">Active</OverlineTitle></th>
                  ...
                  <TableBody>
                    <tr v-for="(user, index) in users" v-bind:key="index">
                        ...
                        <td>{{ user.phone }}</td>
                        <td>
                          <Badge v-if="user.active == 'Yes'" soft variant="success">Yes</Badge>
                          <Badge v-if="user.active == 'No'" soft variant="danger">No</Badge>
                        </td>
                        <td class="d-flex justify-content-end">
                          <router-link :to="{ name: 'EditTenant', params: { id: user.id } }">
  ...
  export default {
    ...
    methods: {
      fetchUsers(){
        ...
        axios.get(this.baseURL+'/api/admin/tenants', { headers })
        .then(response => {
      ...
      deleteRecord(e)
      {
        ...
        axios.post(this.baseURL+'/api/admin/tenants/delete', data, { headers })
        .then(response => {
          if(response.data.status)
          {
