      ...
      <Col xxl="12">
        <CardBody class="flex-grow-0 py-2">
          <CardTitleGroup>
            ...
            <CardTools>
              <Button type="button" variant="info" soft class="mx-1" style="float:right;" as="RouterLink" to="/admin/bedroom/create">
                <Icon icon="plus"/>  
                Create
              </Button> 
            </CardTools>
                ...
                <TableBody>
                  <tr v-for="(bedroom, index) in bedrooms" v-bind:key="index">
                      ...
                      <td class="d-flex justify-content-end">
                        <router-link :to="{ name: 'EditBedroom', params: { id: bedroom.id } }">
                          <Button type="button" variant="primary" soft>Edit</Button> 
                        </router-link>
                          <Button type="button" variant="danger" soft class="mx-2" :id="`del-`+bedroom.id">Delete</Button> 
                      </td>
                  </tr>
                </TableBody>
              </DataTable>
...
export default {
  ...
  methods: {
    ...
    deleteRecord(e)
    {
      ...
      axios.post(this.baseURL+'/api/admin/bedroom/delete', data, { headers })
      .then(response => {
        if(response.data.status)
        {
