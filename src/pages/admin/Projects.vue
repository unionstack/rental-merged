    ...
    <Row utils="g-gs">
      <Col xxl="12">
        <CardBody class="flex-grow-0 py-2">
          <div class="alert alert-success" v-if="showAlert">Record deleted successfully!</div>
          <CardTitleGroup>
            <CardTitle tag="div">
              <h3 class="title mb-3">Projects</h3>
            </CardTitle>
            <CardTools>
              <Button type="button" variant="info" soft class="mx-1" style="float:right;" as="RouterLink" to="/admin/project/create">
                <Icon icon="plus"/>  
                Create
              </Button> 
            </CardTools>
                ...
                <TableBody>
                  <tr v-for="(project, index) in projects" v-bind:key="index">
                      ...
                      <td class="d-flex justify-content-end">
                        ...
                        <Button type="button" variant="danger" soft class="mx-2" :id="`del-`+project.id">Delete</Button> 
                      </td>
                  </tr>
                </TableBody>
              </DataTable>
...
export default {
  ...
  methods: {
    ...
    deleteRecord(e)
    {
      ...
      axios.post(this.baseURL+'/api/admin/project/delete', data, { headers })
      .then(response => {
        if(response.data.status)
        {
