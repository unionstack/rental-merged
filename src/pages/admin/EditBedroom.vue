      ...
      <Row utils="g-gs">
        <Col xxl="12">
          <CardBody class="flex-grow-0 py-2">
            <CardTitleGroup>
              <CardTitle tag="div">
                <h3 class="title">Edit Bedroom</h3>
              </CardTitle>
              <CardTools>
                      ...
                      <div class="col-md-6">
                          <div class="form-group">
                              ...
                              <div class="form-control-wrap">
                                  <FormInput type="text" v-model="form.name" required placeholder="Enter Bedrooms"/>
...
export default {
  ...
  methods: {
    fetchBedroom(){
      ...
      // console.log(this.id);
      axios.get(this.baseURL+'/api/admin/'+this.id+'/edit-bedroom/', { headers })
      .then(response => {
        if(response.data.status)
        {
          this.bedroom_data = response.data.data;
          this.form.name = this.bedroom_data[0].name;
        }
      });
    },
    update(e){
      ...
      axios.post(this.baseURL+'/api/admin/bedroom/'+this.id+'/update', this.form, { headers })
      .then(response => {
        if(!response.data.status)
        {
